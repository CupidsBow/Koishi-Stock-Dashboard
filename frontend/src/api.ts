import type { Candle, IndicatorsResponse, StockInfo } from "./types";

const BASE = "/api";

export async function fetchCandles(symbol: string, days?: number): Promise<Candle[]> {
  let url = `${BASE}/candles?symbol=${encodeURIComponent(symbol)}`;
...
export async function fetchMinutes(symbol: string, period = "5"): Promise<Candle[]> {
  const url = `${BASE}/minutes?symbol=${encodeURIComponent(symbol)}&period=${period}`;
  const res = await fetch(url);
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error((body as { error?: string }).error ?? `HTTP ${res.status}`);
